...
function generateMob () {
	var monster = [];
	var mob1 = ['Blaze ', 'Aqua ', 'Electric ', 'Terra ', 'Wind '];
	var mob2 = ['Phoenix ', 'Dragon ', 'Irfit ', 'Chimera ', 'Were-Wolf '];
	var brackets = ['**('];
	var brackets2 = [')**'];
	var lives = ['\u2665'];
	var life = [1, 2, 3, 4, 5, 6];
	for (var i = 0; i < mob1.length; i++) {
		for (var j = 0; j < mob2.length; j++) {
			for (var k = 0; j < brackets.length; k++) {
				for (var l = 0; j < lives.length; l++) {
					for (var m = 0; j < life.length; m++) {
						for (var n = 0; j < brackets2.length; n++) {
							monster.push({mob1: mob1[i], mob2: mob2[j], brackets: brackets[k], lives: lives[l], life: life[m], brackets2: brackets2[n]});
						}
					}
				}
			}
...
function generateLife () {
	...
	var lives2 = ['\u2665'];
	var life5 = [3];
	for (var i = 0; i < brackets3.length; i++) {
		for (var j = 0; j < brackets3.length; j++) {
			for (var k = 0; j < lives2.length; k++) {
				for (var l = 0; j < life5.length; l++) {
					life2.push({brackets3: brackets3[i], lives2: lives2[j], life5: brackets4[k]});
				}
			}
		}
	}
	return life2.randomize()
}

function generateTarget () {
	var targets = [];
	var players = [this.players];
	return targets.randomize()
}

var TheShootingGame = exports.TheShootingGame = (function () {
	function TheShootingGame (opts, output) {
		this.output = output;
		this.room = opts.room || '';
		this.title = opts.title || 'TheShootingGame';
		this.monsters = generateMob;
		this.target1 = generateTarget;
		this.roundTime = opts.roundTime || 9500;
		this.status = 0; // 0 - off, 1 - singups, 2 - started
		this.players = {};
		this.timer = null;
	...
	TheShootingGame.prototype.emit = function (type, data) {
		if (typeof this.output === "function") return this.output.call(this, type, data);
	};

	TheShootingGame.prototype.init = function () {
		this.singups();
	};

	TheShootingGame.prototype.singups = function () {
		this.clearTimers();
		this.status = 1;
		this.players = {};
		this.emit('singups');
	...
	TheShootingGame.prototype.userJoin = function (user) {
		if (this.status !== 1) return;
		this.players[toId(user)] = user;
	};

	TheShootingGame.prototype.userLeave = function (user) {
		if (this.status !== 1) return;
		if (this.players[toId(user)]) delete this.players[toId(user)];
	};

	TheShootingGame.prototype.start = function () {
		if (this.status !== 1) return;
		var players = [];
		for (var i in this.users) {
			players.push({id: i, name: this.users[i].substr(1), life2: []});
		}
		if (Object.keys(this.players).length < 2) return;
		this.status = 2;
		this.round = 0;
		this.getMonsters = [this.getMob(), this.getMob()];
		this.reportRound();
		this.timer = setInterval(this.reportRound.bind(this), this.roundTime);
	};
	
	TheShootingGame.prototype.getMob = function () {
	    this.monster = generateMob()
	}

	TheShootingGame.prototype.reportRound = function () {
		if (this.status !== 2) return;
		this.round++;
		var players = [];
		for (var p in this.players) players.push(this.players[p]);
		...
		this.emit('shoot', this.getTarget(), this.getTarget())
		if (players.length < 2) return;
		this.emit('round', {round: this.round, players: players});
	};

	TheShootingGame.prototype.getTarget = function () {
		this.targets = generateTarget ()
	};

	TheShootingGame.prototype.shoot = function (killer, enemy, life) {
		if (this.status !== 2) return;
		killer = toId(killer);
		enemy = toId(enemy);
		if (!this.players[killer] || !this.monster[enemy]) return;
		this.life-1;
		if (Object.life(this.enemy).length === 0) return;
		delete this.monster[enemy];
		if (Object.keys(this.players).length < 2) this.end(this.players[killer]);
	};

	TheShootingGame.prototype.end = function (winner) {
		this.clearTimers();
		this.status = 0;
		this.emit('end', winner);
	};

	TheShootingGame.prototype.forceend = function () {
		this.clearTimers();
		this.status = 0;
		this.emit('forceend');
	};

	TheShootingGame.prototype.clearTimers = function () {
		if (this.timer) {
			clearInterval(this.timer);
			this.timer = null;
		}
