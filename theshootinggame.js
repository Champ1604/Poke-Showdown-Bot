...
var TheShootingGame = require('./constructors.js').TheShootingGame;
...
function trans (data, room) {
	...
	return Tools.translateGlobal('games', 'TheShootingGame', lang)[data];
}

exports.id = 'TheShootingGame';

exports.title = 'The Shooting Game';

exports.aliases = ['shooting', 'shoot', 'tsg', 'shootinggame'];
...
var parser = function (type, data) {
	switch (type) {
		...
		case 'round':
			...
			sendML(this.room, "**" + exports.title + trans("monsters1", this.room)) + "attacks" + trans("targets1", this.room) + "and does 1 life damage!" + "**"
			break;
		case 'end':
...
exports.newGame = function (room, opts) {
	...
	var game = new TheShootingGame(generatorOpts, parser);
	if (!game) return null;
...
exports.commands = {
	...
	shoot: function (arg, by, room, cmd, game) {
		game.shoot(by, arg);
	},
	end: 'endTheShootingGame',
	endTheShootingGame: function (arg, by, room, cmd, game) {
		if (!this.can('games')) return;
		game.forceend();
	}
};
